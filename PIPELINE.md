...
12. Run `check_embeddings.py` with the constant inside script `DELETE_WRONG_EMBEDDINGS = 0`
13. If sript find wrong embeddings, run this script with `DELETE_WRONG_EMBEDDINGS = 1` , and after that re-make embedding (step **11**)
14. Run `validate_model.py` - it validates the model. If all tests passed, the model works. If not, you can contact the model developers.